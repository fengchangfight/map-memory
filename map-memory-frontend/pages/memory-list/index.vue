<template>
  <div class="memory-list-page">
    ...
    <el-dialog :visible.sync="unlockReadCodeVisible" :fullscreen="false" title="解锁" width="30%">
    ...
    <div id="list-box" v-loading="list_loading" class="center-content">
      <div style="display:flex;margin-top:5px;width:100%;" @keyup.enter="filterMemory">
        <el-input style="width:60%;" v-model="query" placeholder="按标题或内容查找" />
        <el-button style="width:15%;" type="primary" @click="filterMemory">过滤</el-button>
        <el-select style="width:25%;" v-model="sort_by" placeholder="排序方式" @change="changeSortBy()">
          <el-option
      ...
      <el-table :border="true" :data="memorylistdata" style="width: 100%">
          ...
          <template slot-scope="scope" v-if="scope.row">
            <div style="display:flex;">
              <button
                ...
                v-bind:style="{'backgroundImage': 'url(/imgs/' + scope.row.icon + ')', 'background-size': '100% 100%',  'width': '50px', 'height': '50px' }"
              ></button>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="标题" prop="title" width="168">
          <template slot-scope="scope" v-if="scope.row">
            <img
        ...
        <el-table-column label="经度" prop="longitude" width="90" />
        <el-table-column label="纬度" prop="latitude" width="100" />
        <el-table-column label="创建/最后更新时间" width="165">
          <template
            slot-scope="scope"
            v-if="scope.row"
          >{{ scope.row.created_at }}/{{ scope.row.last_update }}</template>
        </el-table-column>
          ...
          <template slot-scope="scope" v-if="scope.row">
            <div style="display:flex;">
              ...
              <el-button
                size="small"
                type="primary"
                @click="openMemPoint(scope.row.id, scope.row.locked)"
              >打开</el-button>
